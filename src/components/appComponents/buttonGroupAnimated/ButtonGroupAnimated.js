...
export default function AnimatedGroupButton({
  ...
  const defaultActiveTintColor = activeTintColor ? activeTintColor : APP_STYLES.textPrimaryColor.color
  const defaultInactiveTintColor = inActiveTintColor ? inActiveTintColor : APP_COLORS.appTextColor5
          ...
          {data.map((item, key) => {
            ...
            return (
              <TouchableHighlight
                ...
                underlayColor={APP_COLORS.transparent}
                onLayout={event => {
                  (item.x = event.nativeEvent.layout.x),
                    (item.tabHeight = event.nativeEvent.layout.height),
                    (item.tabWidth = event.nativeEvent.layout.width)
                ...
                style={[
                  {
                    ...
                    marginLeft: key === 0 ? APP_SIZES.marginHorizontal : 0,
                    marginRight: APP_SIZES.marginHorizontal / 2
                  },
                  inActiveButtonStyle,
                      ...
                      }
                      {
                        data[selectedTabIndex].icon ?
                          <Icons.Custom
                            ...
                            color={activeTintColor ? activeTintColor : APP_STYLES.textWhite.color}
                            size={iconSize ? iconSize : defaultIconSize}
                            style={[!hideLabel && styles.iconStyle, iconStyle]}
                          />
                          :
...
const styles = StyleSheet.create({
  ...
  animatedGroupButtonInactivatedButton: {
    borderRadius: APP_SIZES.buttonRadius,
  ...
  animatedGroupButtonActivatedButtonTxt: {
    ...APP_STYLES.textWhite
  },
  iconStyle: { marginLeft: APP_SIZES.marginHorizontal / 4 }
})